...
public class Job {
	...
	public Job() {
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Geometry g = ifc.getGeometryBySqlGeometry(r
						.getObjectValue("GEOM"));
			...
			for (int i = 0; i < ships.length(); i++) {
				...
				if (apply.has(mmsi)) {
					...
					for (int j = 0; j < this.berth.length(); j++) {
						...
						boolean ok = false;
						ok = new Date().compareTo(mt) <= 0 ? !code.equals(cb)
								&& !code.equals(tb) : !code.equals(tb);
						if (ok) {
							JSONArray temp = (JSONArray) bh
									.getJSONObject("GEOM")
									.getJSONArray("rings").get(0);
							boolean flag = isPtInPoly(
									ship.getDouble("LONGITUDE"),
									ship.getDouble("LATITUDE"), temp);
							if (flag) {
	...
	// stage.startEdit();
	// String sql = "SELECT * FROM IMAP_WARNING_12 WHERE ISHANDLED='N'";
	// long toal = stage.getDb().executeSqlForTotal(sql);
	// String update = "UPDATE IMAP_WARNING_NEW SET WARNING12=%S";
	...
	// stage = getWorkspace("stage");
	// stage.startEdit();
	// String sql = "SELECT * FROM IMAP_WARNING_13 WHERE ISHANDLED='N'";
	// long toal = stage.getDb().executeSqlForTotal(sql);
	// String update = "UPDATE IMAP_WARNING_NEW SET WARNING13=%S";
	// stage.getDb().executeSqlNoQuery(String.format(update, toal));
	// stage.endEdit();
	// } catch (Exception e) {
	// // TODO Auto-generated catch block
	...
	//
	// /**
	// * 监控是否有新增的调拨车辆运行超时异常报警
	// *
	// * @throws EcityException
	// */
	// @Scheduled(cron = "0 */2 * * * ?")
	// public void job4() throws EcityException {
	// IWorkspace stage = null;
	// try {
	// stage = getWorkspace("stage");
	// stage.startEdit();
	// String sql = "SELECT * FROM IMAP_WARNING_14 WHERE ISHANDLED='N'";
	// long toal = stage.getDb().executeSqlForTotal(sql);
	// String update = "UPDATE IMAP_WARNING_NEW SET WARNING14=%S";
	// stage.getDb().executeSqlNoQuery(String.format(update, toal));
	// stage.endEdit();
	// } catch (Exception e) {
	// // TODO Auto-generated catch block
	...
	// */
	// @Scheduled(cron = "0 */2 * * * ?")
	// public void job5() throws EcityException {
	// IWorkspace stage = null;
	// try {
	// stage = getWorkspace("stage");
	// stage.startEdit();
	// String sql = "SELECT * FROM IMAP_WARNING_18 WHERE ISHANDLED='N'";
	// long toal = stage.getDb().executeSqlForTotal(sql);
	// String update = "UPDATE IMAP_WARNING_NEW SET WARNING18=%S";
	// stage.getDb().executeSqlNoQuery(String.format(update, toal));
	// stage.endEdit();
	// } catch (Exception e) {
	// // TODO Auto-generated catch block
	...
	//
	// /**
	// * 监控是否有新增的行政通道车辆布控中控异常报警
	// *
	// * @throws EcityException
	// */
	// @Scheduled(cron = "0 */2 * * * ?")
	// public void job6() throws EcityException {
	// IWorkspace stage = null;
	// try {
	// stage = getWorkspace("stage");
	// stage.startEdit();
	// String sql = "SELECT * FROM IMAP_WARNING_19 WHERE ISHANDLED='N'";
	// long toal = stage.getDb().executeSqlForTotal(sql);
	// String update = "UPDATE IMAP_WARNING_NEW SET WARNING19=%S";
	// stage.getDb().executeSqlNoQuery(String.format(update, toal));
	// stage.endEdit();
	// } catch (Exception e) {
	// // TODO Auto-generated catch block
	 ...
	 * @return
	 */
	public boolean isPtInPoly(double ALon, double ALat, JSONArray APoints)
	 ...
	 * @param errBerth
	 */
	public void addWarning(JSONObject ship, String croBerth, String errBerth)
			throws EcityException {
		try {
			...
			workspace.startEdit();
			workspace.getDb().executeSqlNoQuery(
					"UPDATE IMAP_WARNING_NEW SET WARNING20 = WARNING20 + 1");
			ITableClass itc = workspace.getTableClass("IMAP_WARNING_20");
	...
	public IWorkspace getWorkspace(String name) {
		...
		try {
			...
			conn.dbInfo = new DB_INFO("oracle", "172.28.1.102:1521:hgdb", un,
					pw, 10, 10);
